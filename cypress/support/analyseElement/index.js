...
Cypress.Commands.add("base64screenshot", function(element) {
  return cy.wrap(null).then(() => {
    ...
    return cy.get(element).first().screenshot("screenshot").then(() => {
      return cy.task("getFile", ["cypress", "screenshots", "screenshot.png"]).then(data => {
...
function getImageData(base64image, startX = 0, startY = 0, width, height) {
  ...
  return cy.waitUntil(() => loaded === true).then(() => {
...
Cypress.Commands.add("analyseImage", function(base64Image, startX, startY, width, height) {
 ...
 */
Cypress.Commands.add("analyseElement", function (element, startX, startY, width, height) {
  cy.wait(100);
